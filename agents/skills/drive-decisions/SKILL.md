...
- 観点・対応案を列挙するときは、必須 / 推奨 / 任意 の3段階ラベルを付ける。並列のフラットなリストにしない(ユーザーは「どれを今やるか」を切る判断をしたい)
- 「任意」と判断したものをユーザーが「やらない」と切ったら、成果物からも移動・削除する(scope-boundaries)
...
- 推奨・判断軸なしの選択肢の列挙
- 必須度(必須/推奨/任意)を付けないフラットな観点・対応案の列挙
- 変更量の少なさだけを根拠にした推奨
